...
import { DetectionResult } from "@/lib/types/DetectionResult";
import calculateTOPSIS from "@/utils/topsis-yolo";
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          ...
          try {
            const result = await getStreetNameFromOverpass(
              newCoords.lat,
              newCoords.lng
            );
            setLocationInfo({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!capturedImage || !coords) {
      toast({
        ...
        description: "Please capture an image and ensure location is set.",
      });
      return;
    }
    setIsLoading(true);
    ...
    try {
      ...
      // membuat nama file yang unik untuk menyimpan label dari yolo
      const uniqueFileName = `captured-photo-${new Date().getTime()}.jpeg`;
      // mengubah url menjadi file
      const photo: File = urlToFile(capturedImage, uniqueFileName);
      // membuat form data berisi file gambar untuk di kirim ke deteksi yolo
      ...
      // mendapatkan hasil json dari api deteksi yolo
      const resultYolo = await response.json();
      // mendapatkan file label yang telah dikirim deteksi yolo
      const fetchLabel = await fetch(resultYolo.labelPath);
      // mengambil isi file label dalam bentuk json
      const dataLabel = await fetchLabel.json();
      const processedResults = dataLabel.detections.map(
        (detection: DetectionResult) => {
          ...
          const polygonCoords = detection.polygon;

          const area =
            (calculatePolygonArea(polygonCoords) / (640 * 640)) * 100;
      ...
      // proses topsis untuk menghitung skor per kriteria
      const topsis = calculateTOPSIS(processedResults);
      // mendapat skor akhir dari output yolo
      ...
      await addReport({
        image: capturedImage,
        labelPath: resultYolo.labelPath,
        scoreDamage: finalScore,
      ...
      toast({
        ...
        description: "Could not submit your report. " + error,
      });
      setIsLoading(false);
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex items-center justify-between">
                ...
                <Badge
                  variant={
                    !locationInfo.confidence
                      ? "secondary"
                      : locationInfo.confidence >= 0.7
                      ? "default"
                      : locationInfo.confidence >= 0.3
                      ? "secondary"
                      : "destructive"
                  }
                  className="text-xs"
      ...
      <Button
        type="submit"
        className="w-full"
        disabled={isLoading || !capturedImage || !coords}
      >
        {isLoading ? (
...
async function fetchRoadInfo(lat: number, lon: number) {
  ...
  const way = data.elements.find(
    (el: any) => el.type === "way" && el.tags && el.tags.name
  );
  if (!way) return null;
